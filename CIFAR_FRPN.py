...
def convergence(pooling, stato, stato_vecchio, k, y_con, c):
  ...
  h_concat=tf.concat([pooling,stato],3)
    
  # CONVOLUTION=STATE

  h_conv2 = tf.nn.tanh(conv2d(h_pool1, W_conv2) + b_conv2)
  h_conv2r = tf.reshape(h_conv2, [batch_size, 16, 16, 4]) 
  ...
  # SOFTMAX 
  y_con = tf.reshape(tf.nn.softmax(tf.matmul(h_fc1, W_fc2) + b_fc2), [batch_size, 10])
  ...
  # INCREMENT COUNTER
  k=k+1
  return pooling, h_conv2r, stato, k, y_con, c
...
def condition(a, stato, stato_vecchio, k, y_con, c):
  ...
  # CONDITION
  c1 = tf.greater(outDistance, 0.5)# CHENGE THIS PARAMETER!!
...
d1.to_csv("accuracy-conv123.csv", index=False)
d2.to_csv("loss-conv123.csv", index=False)
d3.to_csv("cicli-conv123.csv", index=False)
d4.to_csv("entropy-conv123.csv",index=False)
d5.to_csv("validation-conv123.csv",index=False)
## DATI RELATIVI AL TEST SET
d11=pd.DataFrame(predic)
d22=pd.DataFrame(cicle_test)
d11.to_csv("prediction-conv123.csv", index=False)
d22.to_csv("cicli_test-conv123.csv", index=False)
